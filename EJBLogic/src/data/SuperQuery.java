...
/**
 * 超完美jpa动态查询封装
 * 参考地址：http://www.oschina.net/code/snippet_1864608_37194?mType=Group
...
public class SuperQuery implements Serializable
{
    ...
    /** 关联模式 */
    private Map<String, SuperQuery> subQuery;

    private Map<String, SuperQuery> linkQuery;

    private String projection;

    /** 或条件 */
    private List<SuperQuery> orQuery;
    ...
    private SuperQuery(Class clazz, EntityManager entityManager) {
        this.clazz = clazz;
    ...
    /** 通过类创建查询条件 */
    public static SuperQuery forClass(Class clazz, EntityManager entityManager) {
        return new SuperQuery(clazz, entityManager);
    }

    /** 增加子查询 */
    private void addSubQuery(String propertyName, SuperQuery query) {
    ...
    private void addSubQuery(SuperQuery query) {
        addSubQuery(query.projection, query);
    }

    /** 增关联查询 */
    public void addLinkQuery(String propertyName, SuperQuery query) {
    ...
    private void addLinkCondition(SuperQuery query) {
        ...
        for (Iterator queryIterator = subQuery.keySet().iterator(); queryIterator.hasNext();) {
            ...
            SuperQuery sub = (SuperQuery) subQuery.get(key);
            from.join(key);
